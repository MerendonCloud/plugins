...
Identities are mailboxes on your own domain, not shared inboxes — a desk address like `stashrelay@` is a single identity that an agent reaches only through an MCP grant, never through membership. The two gates — inbound release and outbound approval — are the product, not a limitation.
...
| `list_sent` | List outbound messages that have actually been sent for an identity. |
| `read_thread` | Read the released messages in a thread. |
| `get_attachment` | Get metadata and a download link for an attachment on a thread you can already read. |
